...
def filter_data(request):
    ...
    if 'newCourses' in filters:
        courses = courses.filter(price__gte=1).order_by('-id')

    if 'priceLowToHigh' in filters:
        courses = courses.filter(price__gte=1).order_by('price')

    if 'priceHighToLow' in filters:
        courses = courses.filter(price__gte=1).order_by('-price')

    rendered_courses = render_to_string('ajax/course.html', {'course': courses})
    return JsonResponse({'data': rendered_courses, 'course_count': courses.count()})
...
def add_to_group(request, formateur_id):
    ...
    if user.groups.filter(name='Formateur').exists():
        ...
        message = f'Dear {formateur.name},\n\nYour account has been created successfully. You can now access our platform as a formateur.\n Email : {formateur.email_author} \n Password : 0000'
        sender_email = 'abdelhakaitkadir@gmail.com'  
        send_mail(subject, message, sender_email, [formateur.email_author])
